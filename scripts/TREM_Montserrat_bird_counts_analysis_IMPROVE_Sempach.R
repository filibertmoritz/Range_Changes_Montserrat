...
SPECIES <- c('TREM') # fill in SPECIES the analysis should be made for
...
fm28 <- colext(~treeheight + canopy, ~1, ~1, ~day+time+rain+wind+activity+location, data = umf, se = T) # constant model
fm29 <- colext(~treeheight + canopy, ~alt, ~1, ~day+time+rain+wind+activity+location, data = umf, se = T) # expansion model
fm30 <- colext(~treeheight + canopy, ~1, ~alt, ~day+time+rain+wind+activity+location, data = umf, se = T) # contraction model 
fm31 <- colext(~treeheight + canopy, ~alt, ~alt, ~day+time+rain+wind+activity+location, data = umf, se = T) # shift model
fm32 <- colext(~treeheight + canopy, ~year_num, ~year_num, ~day+time+rain+wind+activity+location, data = umf, se = T)  # year_num model (trend), this model will exclude the possibility that observed changes are just annual changes
fm33 <- colext(~treeheight + canopy, ~year_fact, ~year_fact, ~day+time+rain+wind+activity+location, data = umf, se = T)  # year_fact model, this model will exclude the possibility that observed changes are just annual changes
# correct for year_num (trend) and explore alt effects
fm34 <- colext(~treeheight + canopy, ~year_num+alt, ~year_num, ~day+time+rain+wind+activity+location, data = umf, se = T)  # corrected year_num - expansion
fm35 <- colext(~treeheight + canopy, ~year_num, ~year_num+alt, ~day+time+rain+wind+activity+location, data = umf, se = T)  # corrected year_num - contraction
fm36 <- colext(~treeheight + canopy, ~year_num+alt, ~year_num+alt, ~day+time+rain+wind+activity+location, data = umf, se = T)  # corrected year_num - shift
# correct for year_fact and explore alt effects
fm37 <- colext(~treeheight + canopy, ~year_fact+alt, ~year_fact, ~day+time+rain+wind+activity+location, data = umf, se = T)  # corrected year_fact - expansion
fm38 <- colext(~treeheight + canopy, ~year_fact, ~year_fact+alt, ~day+time+rain+wind+activity+location, data = umf, se = T)  # corrected year_fact - contraction
fm39 <- colext(~treeheight + canopy, ~year_fact+alt, ~year_fact+alt, ~day+time+rain+wind+activity+location, data = umf, se = T)  # corrected year_fact - shift, also  global model
...
# best sub-model for g_e(): contraction ~1, ~alt, AICc difference to second best shift is 3.21
