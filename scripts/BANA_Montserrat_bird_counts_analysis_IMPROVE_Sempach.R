...
(p_modSel_df <- aictab(cand.set = p_fitList, c.hat = c_hat_pb) %>% # create a model comparison table with QAICc or AICc depending on c-hat from gof
  mutate(step = 'p'))
# best submodel for p():~day+time+rain+wind+activity, AICc difference to second best 0.67 - go on with this fm7 best one
...
(psi_modSel_df <- aictab(cand.set = psi_fitList, c.hat = c_hat_pb) %>% 
    mutate(step = 'psi'))
# best sub-model for psi(): ~1 has lowest AICc, with an AICc difference to to 2nd best model of 2.62 (~alt)
