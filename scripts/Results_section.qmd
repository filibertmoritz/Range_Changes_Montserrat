...
Presence-absence point count data on `r length(SPECIES)` forest bird species on Montserrat was sampled over `r unique(occu$Year) %>% length()` years between `r min(occu$Year)` and `r max(occu$Year)`. Consequently, we analysed `r countdata %>% filter(VisitID != 'NA') %>% filter(!Point %in% c(99,76)) %>% nrow() * length(SPECIES)` observations from `r countdata %>% filter(VisitID != 'NA') %>% filter(!Point %in% c(99,76)) %>% nrow()` surveys using dynamic occupancy models that correct for imperfect detection, calculate colonisation and extinction probability and provide the possibility to estimate yearly occupancy. We found evidence for elevational range changes in `r length(unique(pred_colext$species))` out of the `r length(SPECIES)` analysed species (Figure 2, Table 3), with `r modsel_best %>% filter(model == 'contraction') %>% nrow()` species (Bridled Quail-Dove, Scaly-Breasted Thrasher and Brown Trembler) best explained by the ‘contraction’ model where extinction depended on elevation, `r modsel_best %>% filter(model == 'expansion') %>% nrow()` species (Purple-Throated Carib) where extinction was explained by elevation ('expansion' model), and `r modsel_best %>% filter(model == 'shift') %>% nrow()` species (Montserrat Oriole) where both colonisation and extinction were explained by elevation ('shift' model). For another `r modsel_best %>% filter(model == 'constant') %>% nrow()` species (Banaquit, Forest Thrush, Pearly-Eyed Thrasher and Scaly-Naped Pigeon) the most parsimonious model was the ‘constant’ model where both colonisation and extinction were modeled with intercept only, and for `r modsel_best %>% filter(model == 'year_num') %>% nrow()` species (Antillean Crested Hummingbird, Caribbean Eleania and Green-Throated Carib) both extinction and colonisation were best explained by year ('year_num' model). The GOF test (n = 1000 simulations, Table SUMMARY TABLE) indicated Chisquare p-values ranging from `r min(gof_df$Chisq_p)` to `r max(gof_df$Chisq_p)` . C-hat values for all models laid below 1, but underdispersion was negligible (c-hat \> 0.9, SOURCE) for `r gof_df %>% filter(c_hat > 0.9) %>% nrow()` . Thus, `r gof_df %>% filter(c_hat < 0.9) %>% nrow()` models showed c-hat values substantially lower than 1, potentially indicating a lack-of-fit that may affect the validity of the models for inference. Moreover, model selection uncertainty occurs in the most parsimonious models for `r modsel %>% filter(step == 'g_e') %>% filter(Delta_AICc <=2) %>% filter(Delta_AICc !=0) %>% group_by(species) %>% count() %>% nrow()` out of `r length(SPECIES)` analysed species including `r intersect(modsel_best %>% filter(model %in% c('shift', 'contraction', 'expansion', 'year_num_shift', 'year_num_contraction', 'year_num_expansion', 'year_fact_shift', 'year_fact_expansion', 'year_fact_contraction')) %>% pull(species), modsel %>% filter(step == 'g_e') %>% filter(Delta_AICc <2) %>% filter(Delta_AICc !=0) %>% group_by(species) %>% count() %>% select(species) %>% pull(species)) %>% length()` (Bridled-Quail Dove and Purple-Throated Carib) out of `r length(modsel_best %>% filter(model %in% c('shift', 'contraction', 'expansion', 'year_num_shift', 'year_num_contraction', 'year_num_expansion', 'year_fact_shift', 'year_fact_expansion', 'year_fact_contraction')) %>% pull(species))` models which suggest elevational range changes. For these models ΔAIC lies within 2 AIC units and therefore the alternative models are not clearly distinguishable from the most parsimonious one (Burnham and Anderson, 2002).
...
For the Montserrat Oriole colonisation increased with elevation from `r min(pred_colext %>% filter(species == 'MTOR') %>% filter(Type == 'Colonisation') %>% pull(Predicted))` to `r max(pred_colext %>% filter(species == 'MTOR') %>% filter(Type == 'Colonisation') %>% pull(Predicted))` and extinction decreased with elevation from `r max(pred_colext %>% filter(species == 'MTOR') %>% filter(Type == 'Extinction') %>% pull(Predicted))` to `r min(pred_colext %>% filter(species == 'MTOR') %>% filter(Type == 'Extinction') %>% pull(Predicted))` showing effect sizes of `r coeff_bm %>% filter(species=='MTOR') %>% filter(component =='col(alt)') %>% pull(estimate)` (0.95 CI: `r coeff_bm %>% filter(species=='MTOR') %>% filter(component =='col(alt)') %>% pull(lower)`, `r coeff_bm %>% filter(species=='MTOR') %>% filter(component =='col(alt)') %>% pull(upper)`) and `r coeff_bm %>% filter(species=='MTOR') %>% filter(component =='ext(alt)') %>% pull(estimate)` (0.95 CI: `r coeff_bm %>% filter(species=='MTOR') %>% filter(component =='ext(alt)') %>% pull(lower)`, `r coeff_bm %>% filter(species=='MTOR') %>% filter(component =='ext(alt)') %>% pull(upper)`), respectively. Notably, the steepness of both colonisation and extinction slopes was highest for low elevations but flattening towards higher elevations. The ‘expansion’ model for the Purple-Throated Carib suggested a positive colonisation-elevation relation of `r coeff_bm %>% filter(species == 'PTCA') %>% filter(component == 'col(alt)') %>% pull(estimate)` (0.95 CI: `r coeff_bm %>% filter(species=='PTCA') %>% filter(component =='col(alt)') %>% pull(lower)` , `r coeff_bm %>% filter(species=='PTCA') %>% filter(component =='col(alt)') %>% pull(upper)`) leading to values between `r min(pred_colext %>% filter(species == 'PTCA') %>% filter(Type == 'Colonisation') %>% pull(Predicted))` and `r max(pred_colext %>% filter(species == 'PTCA') %>% filter(Type == 'Colonisation') %>% pull(Predicted))`. The models for the Brown Trembler and Bridled Quail-Dove indicated decreasing extinction along elevation (ranging from `r max(pred_colext %>% filter(species == 'TREM') %>% filter(Type == 'Extinction') %>% pull(Predicted))` - `r min(pred_colext %>% filter(species == 'TREM') %>% filter(Type == 'Extinction') %>% pull(Predicted))` and `r max(pred_colext %>% filter(species == 'BRQD') %>% filter(Type == 'Extinction') %>% pull(Predicted))` - `r min(pred_colext %>% filter(species == 'BRQD') %>% filter(Type == 'Extinction') %>% pull(Predicted))`) but small effect sizes of `r coeff_bm %>% filter(species=='TREM') %>% filter(component =='ext(alt)') %>% pull(estimate)` (0.95 CI: `r coeff_bm %>% filter(species=='TREM') %>% filter(component =='ext(alt)') %>% pull(lower)`, `r coeff_bm %>% filter(species=='TREM') %>% filter(component =='ext(alt)') %>% pull(upper)`), and `r coeff_bm %>% filter(species=='BRQD') %>% filter(component =='ext(alt)') %>% pull(estimate)` (0.95 CI: `r coeff_bm %>% filter(species=='BRQD') %>% filter(component =='ext(alt)') %>% pull(lower)`, `r coeff_bm %>% filter(species=='BRQD') %>% filter(component =='ext(alt)') %>% pull(upper)`), respectively. Again, extinction flattened considerably towards higher elevations. Contrastingly, for the Scaly-Breasted Thrasher extinction generally increased with elevation spanning from `r min(pred_colext %>% filter(species == 'SBTH') %>% filter(Type == 'Extinction') %>% pull(Predicted))` to `r max(pred_colext %>% filter(species == 'SBTH') %>% filter(Type == 'Extinction') %>% pull(Predicted))` and the estimated effect size was `r coeff_bm %>% filter(species == 'SBTH') %>% filter(component == 'ext(alt)') %>% pull(estimate)` (0.95 CI: `r coeff_bm %>% filter(species=='SBTH') %>% filter(component =='ext(alt)') %>% pull(lower)`, `r coeff_bm %>% filter(species=='SBTH') %>% filter(component =='ext(alt)') %>% pull(upper)`). All effect sizes were reported on the logit-scale.
...
plot_colext <- pred_colext %>% 
  ...
  theme_bw()
print(plot_colext)
ggsave(filename = 'C:/Users/filib/Documents/Praktika/Sempach/Montserrat/Range_Changes_Montserrat/output/plot/Colext_dynamics.jpg', 
       plot = last_plot(), scale = 1, dpi = 'retina', units = 'px', 
       width = 3000, height = 2200)
```

### Occupancy \[delete? - otherwise the text has to be updated\]
...
```{r, include=F}
#| label: Occupancy trajectory plot
...
plot_occu <- occu %>% 
  ...
  theme_test()
print(plot_occu)
ggsave(filename = 'C:/Users/filib/Documents/Praktika/Sempach/Montserrat/Range_Changes_Montserrat/output/plot/Occupancy_trajectories.jpg', 
       plot = last_plot(), scale = 1, dpi = 'retina', units = 'px', 
       width = 3000, height = 2200)
```
...
### Appendix 2: Model selection table for all species and models
