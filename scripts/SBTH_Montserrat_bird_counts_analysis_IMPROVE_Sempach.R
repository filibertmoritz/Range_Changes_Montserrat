...
(p_modSel_df <- aictab(cand.set = p_fitList, c.hat = c_hat_pb) %>% # create a model comparison table with QAICc or AICc depending on c-hat from gof
  mutate(step = 'p'))
# best submodel for p():~day+time+rain+wind+activity+loaction, AICc difference to second best 12.80 - go on with this fm8 best one
...
(psi_modSel_df <- aictab(cand.set = psi_fitList, c.hat = c_hat_pb) %>% 
    mutate(step = 'psi'))
# best sub-model for psi(): ~alt:treeheight, AICc difference to second best of 0.02 (~dbh) and third best with AICc difference of 0.14 (~alt)
...
fm32 <- colext(~alt:treeheight, ~year_num, ~year_num, ~day+time+rain+wind+activity+location, data = umf, se = T)  # year_num model (trend), this model will exclude the possibility that observed changes are just annual changes
...
# best sub-model for g_e(): contraction (~1, ~alt) AICc difference to second best constant is 2.97
...
# as col is constant, prediction for col is not needed
