...
SPECIES <- c('GTCA') # fill in SPECIES the analysis should be made for
...
(p_modSel_df <- aictab(cand.set = p_fitList, c.hat = c_hat_pb) %>% # create a model comparison table with QAICc or AICc depending on c-hat from gof
  mutate(step = 'p'))
# best submodel for p():~day+time+rain+wind+activity, AICc difference to second best 8,15 - go on with this fm7 best one
...
(psi_modSel_df <- aictab(cand.set = psi_fitList, c.hat = c_hat_pb) %>% 
    mutate(step = 'psi'))
# best sub-model for psi(): ~1, AICc difference to second best of 0.34 (~dbh)
...
# best sub-model for g_e(): year_num (~year_num, ~year_num) AICc difference to second best year_num_contraction is 2.56
