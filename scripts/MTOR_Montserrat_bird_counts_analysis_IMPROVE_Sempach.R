...
(p_modSel_df <- aictab(cand.set = p_fitList, c.hat = c_hat_pb) %>% # create a model comparison table with QAICc or AICc depending on c-hat from gof
  mutate(step = 'p'))
# best submodel for p(): day+I(time^2)+rain+wind+activity+location, AICc difference to second best 12.41 - go on with this fm8 best one

###### 4.2: fit models for initial occupancy psi() first for modSel ####

fm9 <- colext(~alt, ~1, ~1, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)
# add treeheight
fm10 <- colext(~treeheight, ~1, ~1, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)
fm11 <- colext(~alt+treeheight, ~1, ~1, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)
# add dbh
fm12 <- colext(~dbh, ~1, ~1, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)
fm13 <- colext(~alt+dbh, ~1, ~1, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)
fm14 <- colext(~treeheight+dbh, ~1, ~1, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)
fm15 <- colext(~alt+treeheight+dbh, ~1, ~1, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)
# add canopy
fm16 <- colext(~canopy, ~1, ~1, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)
fm17 <- colext(~dbh+canopy, ~1, ~1, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)
fm18 <- colext(~alt+canopy, ~1, ~1, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)
fm19 <- colext(~treeheight+canopy, ~1, ~1, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)
fm20 <- colext(~alt+dbh+canopy, ~1, ~1, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)
fm21 <- colext(~alt+treeheight+canopy, ~1, ~1, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)
fm22 <- colext(~treeheight+dbh+canopy, ~1, ~1, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)
fm23 <- colext(~alt+treeheight+dbh+canopy, ~1, ~1, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)
# add interaction between alt and treeheight
fm24 <- colext(~alt:treeheight, ~1, ~1, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)
fm25 <- colext(~alt:treeheight+dbh, ~1, ~1, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)
fm26 <- colext(~alt:treeheight+canopy, ~1, ~1, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)
fm27 <- colext(~alt:treeheight+dbh+canopy, ~1, ~1, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)
...
(psi_modSel_df <- aictab(cand.set = psi_fitList, c.hat = c_hat_pb) %>% 
    mutate(step = 'psi'))
# best sub-model for psi(): ~alt:treeheight+canopy, AICc difference to second best of 1.51 (~dbh), then AICc difference is 3.06 (~alt:treeheight+dbh+canopy) 

###### 4.3: fit models for extinction and colonisation probability for modSel ####

fm28 <- colext(~alt:treeheight + canopy, ~1, ~1, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T) # constant model
fm29 <- colext(~alt:treeheight + canopy, ~alt, ~1, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T) # expansion model
fm30 <- colext(~alt:treeheight + canopy, ~1, ~alt, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T) # contraction model 
fm31 <- colext(~alt:treeheight + canopy, ~alt, ~alt, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T) # shift model
fm32 <- colext(~alt:treeheight + canopy, ~year_num, ~year_num, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)  # year_num model (trend), this model will exclude the possibility that observed changes are just annual changes
fm33 <- colext(~alt:treeheight + canopy, ~year_fact, ~year_fact, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)  # year_fact model, this model will exclude the possibility that observed changes are just annual changes
# correct for year_num (trend) and explore alt effects
fm34 <- colext(~alt:treeheight + canopy, ~year_num+alt, ~year_num, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)  # corrected year_num - expansion
fm35 <- colext(~alt:treeheight + canopy, ~year_num, ~year_num+alt, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)  # corrected year_num - contraction
fm36 <- colext(~alt:treeheight + canopy, ~year_num+alt, ~year_num+alt, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)  # corrected year_num - shift
# correct for year_fact and explore alt effects
fm37 <- colext(~alt:treeheight + canopy, ~year_fact+alt, ~year_fact, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)  # corrected year_fact - expansion
fm38 <- colext(~alt:treeheight + canopy, ~year_fact, ~year_fact+alt, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)  # corrected year_fact - contraction
fm39 <- colext(~alt:treeheight + canopy, ~year_fact+alt, ~year_fact+alt, ~day+I(time^2)+rain+wind+activity+location, data = umf, se = T)  # corrected year_fact - shift, also  global model
...
# best sub-model for g_e(): shift ~alt, ~alt, AICc difference to second best year_num_shift is 2.31, then AICc difference is 1.77 (~1 ~1+alt) year 
