...
SPECIES <- c('CAEL') # fill in SPECIES the analysis should be made for
...
(p_modSel_df <- aictab(cand.set = p_fitList, c.hat = c_hat_pb) %>% # create a model comparison table with QAICc or AICc depending on c-hat from gof
  mutate(step = 'p'))
# best submodel for p():~day+I(time^2)+rain+wind+activity, AICc difference to second best 0.67 - go on with this fm7 best one
...
(psi_modSel_df <- aictab(cand.set = psi_fitList, c.hat = c_hat_pb) %>% 
    mutate(step = 'psi'))
# best sub-model for psi(): ~treeheight+canopy has lowest AICc, with an AICc difference to thw 2nd best model of 1.23 (~treeheight+dbh+canopy)

###### 4.3: fit models for extinction and colonisation probability for modSel ####

fm28 <- colext(~treeheight+canopy, ~1, ~1, ~day+I(time^2)+rain+wind+activity, data = umf, se = T) # constant model
fm29 <- colext(~treeheight+canopy, ~alt, ~1, ~day+I(time^2)+rain+wind+activity, data = umf, se = T) # expansion model
fm30 <- colext(~treeheight+canopy, ~1, ~alt, ~day+I(time^2)+rain+wind+activity, data = umf, se = T) # contraction model 
fm31 <- colext(~treeheight+canopy, ~alt, ~alt, ~day+I(time^2)+rain+wind+activity, data = umf, se = T) # shift model
fm32 <- colext(~treeheight+canopy, ~year_num, ~year_num, ~day+I(time^2)+rain+wind+activity, data = umf, se = T)  # year_num model (trend), this model will exclude the possibility that observed changes are just annual changes
fm33 <- colext(~treeheight+canopy, ~year_fact, ~year_fact, ~day+I(time^2)+rain+wind+activity, data = umf, se = T)  # year_fact model, this model will exclude the possibility that observed changes are just annual changes
# correct for year_num (trend) and explore alt effects
fm34 <- colext(~treeheight+canopy, ~year_num+alt, ~year_num, ~day+I(time^2)+rain+wind+activity, data = umf, se = T)  # corrected year_num - expansion
fm35 <- colext(~treeheight+canopy, ~year_num, ~year_num+alt, ~day+I(time^2)+rain+wind+activity, data = umf, se = T)  # corrected year_num - contraction
fm36 <- colext(~treeheight+canopy, ~year_num+alt, ~year_num+alt, ~day+I(time^2)+rain+wind+activity, data = umf, se = T)  # corrected year_num - shift
# correct for year_fact and explore alt effects
fm37 <- colext(~treeheight+canopy, ~year_fact+alt, ~year_fact, ~day+I(time^2)+rain+wind+activity, data = umf, se = T)  # corrected year_fact - expansion
fm38 <- colext(~treeheight+canopy, ~year_fact, ~year_fact+alt, ~day+I(time^2)+rain+wind+activity, data = umf, se = T)  # corrected year_fact - contraction
fm39 <- colext(~treeheight+canopy, ~year_fact+alt, ~year_fact+alt, ~day+I(time^2)+rain+wind+activity, data = umf, se = T)  # corrected year_fact - shift, also  global model
...
# best sub-model for g_e(): year_num (~year_num, ~year_num) AICc difference to 2nd best year_num_expansion is 2.82
...
# as col and ext arer constant pred isn't needed
